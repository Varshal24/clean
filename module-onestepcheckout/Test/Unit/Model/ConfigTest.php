...
class ConfigTest extends \PHPUnit\Framework\TestCase
{
    ...
    /**
     * Test for isEnabledGcAndCouponCodeFieldMerge method
     */
    public function testIsEnabledGcAndCouponCodeFieldMerge()
    {
        $value = 1;
        $this->scopeConfigMock->expects($this->any())
            ->method('isSetFlag')
            ->with(Config::XML_PATH_ENABLE_GC_AND_COUPON_CODE_FIELD_MERGE)
            ->willReturn($value);
        $this->assertSame($value, $this->config->isEnabledGcAndCouponCodeFieldMerge());
    }

    /**
     * Test for isEnabledShowPlaceOrderButtonDefault method
     */
    public function testIsEnabledShowPlaceOrderButtonDefault()
    {
        $value = 1;
        $this->scopeConfigMock->expects($this->any())
            ->method('isSetFlag')
            ->with(Config::XML_PATH_SHOW_PLACE_ORDER_BUTTON_DEFAULT)
            ->willReturn($value);
        $this->assertSame($value, $this->config->isEnabledShowPlaceOrderButtonDefault());
    }

    /**
     * Test for isAllowedToChangeProductOptions method
     */
    public function testIsAllowedToChangeProductOptions()
    {
        $value = 0;
        $this->scopeConfigMock->expects($this->any())
            ->method('isSetFlag')
            ->with(Config::XML_PATH_IS_ALLOWED_TO_CHANGE_PRODUCT_OPTIONS)
            ->willReturn($value);
        $this->assertSame($value, $this->config->isAllowedToChangeProductOptions());
    }
