define(
    ...
    function ($, _, utils, registry, validationEnabledFlag) {
        ...
        function onPlaceChanged(id, autoCompleteObject) {
            ...
            $(':mage-awOscFloatLabel input, :mage-awOscFloatLabel select').trigger('awOscForceRefresh');
        }
        ...
        function prepareStreet(addressData, place) {
            ...
            if (place.types && place.types[0] == 'street_address') {
                ...
                template = evaluateStreetAddressFormat(
                    addressData['street'],
         ...
         */
        function evaluateStreetAddressFormat(streetData, formattedAddress, countryId) {
            var templateStrParts = {},
                templateStr = '',
                isStreetNumFirst = false,
                isNeedToAddSubpremise = false
            ;

            if (countryIdListWithSubpremise.includes(countryId)
                && _.has(streetData, 'subpremise')
            ) {
                isNeedToAddSubpremise = true;
                templateStrParts.streetNum = '<%= subpremise %>';
            }

            if (_.has(streetData, 'street_num')) {
                if (isNeedToAddSubpremise) {
                    templateStrParts.streetNum = templateStrParts.streetNum + '/' + '<%= street_num %>';
                } else {
                    templateStrParts.streetNum = '<%= street_num %>';
                }
                if (isStreetAddressStartWithNumber(formattedAddress, streetData, isNeedToAddSubpremise)) {
        ...
        /**
         * Check if street address start with number
         *
         * @param {string} formattedAddress
         * @param {Array} streetData
         * @param {boolean} isNeedToAddSubpremise
         * @returns {boolean}
         */
        function isStreetAddressStartWithNumber(formattedAddress, streetData, isNeedToAddSubpremise) {
            var regExp = new RegExp(
                    '^'
                    + (
                        isNeedToAddSubpremise
                            ? streetData['subpremise']
                            : streetData['street_num']
                    )
                )
            ;

            return formattedAddress.match(regExp) !== null;
        }
