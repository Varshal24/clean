define([
    ...
    return Component.extend({
        ...
        /**
         * Checking if need to create a token for the current version
         *
         * @return {boolean}
         */
        tokenShouldBeCreated: function () {
            var lastVersionWithJsToken = 295,
                currentVersion = window.checkoutConfig.payment['stripe_payments'].module
                    .split(' ')
                    .pop()
                    .replace(/[^+\d]/g, '');

            return currentVersion <= lastVersionWithJsToken;
        }
    });
});
